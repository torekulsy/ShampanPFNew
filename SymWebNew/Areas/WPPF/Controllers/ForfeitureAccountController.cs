...
namespace SymWebUI.Areas.WPPF.Controllers
{
    public class ForfeitureAccountController : Controller
    {
        ...
        public ActionResult _indexResignEmployee(JQueryDataTableParamModel param)
        {
            ...
            var getAllData = _repo.SelectAll_ResignEmployee();
            IEnumerable<ForfeitureAccountVM> filteredData;
            if (!string.IsNullOrEmpty(param.sSearch))
            {
                var searchText = param.sSearch.ToLower();
                var isSearchable1 = Convert.ToBoolean(Request["bSearchable_1"]);
                var isSearchable2 = Convert.ToBoolean(Request["bSearchable_2"]);
                var isSearchable3 = Convert.ToBoolean(Request["bSearchable_3"]);
                var isSearchable4 = Convert.ToBoolean(Request["bSearchable_4"]);
                ...
                filteredData = getAllData.Where(c =>
                       isSearchable1 && (c.Code ?? "").ToLower().Contains(searchText)
                    || isSearchable2 && (c.EmpName ?? "").ToLower().Contains(searchText)
                    || isSearchable3 && (c.Designation ?? "").ToLower().Contains(searchText)
                    || isSearchable4 && (c.Department ?? "").ToLower().Contains(searchText)
                    || isSearchable5 && (c.EmpResignDate ?? "").ToLower().Contains(searchText)
                    );
            }
            else
            {
            ...
            Func<ForfeitureAccountVM, string> orderingFunction = (c =>
                ...
                sortColumnIndex == 3 && isSortable_3 ? c.Designation :
                sortColumnIndex == 4 && isSortable_4 ? c.Department :
                sortColumnIndex == 5 && isSortable_5 ? c.EmpResignDate :
                "");
            var sortDirection = Request["sSortDir_0"]; // asc or desc
            if (sortDirection == "asc")
                filteredData = filteredData.OrderBy(orderingFunction);
