...
{
    public class LoanMonthlyPaymentController : Controller
    {
        public LoanMonthlyPaymentController()
        {
        ...
        LoanMonthlyPaymentRepo _repo = new LoanMonthlyPaymentRepo();
        ...
        public ActionResult Index()
        {
            ...
            LoanMonthlyPaymentVM vm = new LoanMonthlyPaymentVM();
            return View("~/Areas/WPPF/Views/LoanMonthlyPayment/Index.cshtml",vm);
        }
        public ActionResult _index(JQueryDataTableParamVM param)
        {
            ...
            string[] conditionValues = { AreaTypeWPPFVM.TransType };

            var getAllData = _repo.SelectAll(0, conditionFields, conditionValues);
            IEnumerable<LoanMonthlyPaymentVM> filteredData;
            ...
            if (!string.IsNullOrEmpty(param.sSearch))
            {
                ...
                filteredData = getAllData
                   .Where(c => isSearchable1 && c.Code.ToLower().Contains(param.sSearch.ToLower())
                               ...
                               || isSearchable5 && c.ReferenceNo.ToLower().Contains(param.sSearch.ToLower())
                               || isSearchable6 && c.Post.ToString().Contains(param.sSearch.ToLower())
                               );
            }
            else
            {
            ...
            Func<LoanMonthlyPaymentVM, string> orderingFunction = (c => sortColumnIndex == 1 && isSortable_1 ? c.Code :
                                                           sortColumnIndex == 2 && isSortable_2 ? c.TransactionDate :
                                                           sortColumnIndex == 3 && isSortable_3 ? c.Amount.ToString() :
                                                           sortColumnIndex == 4 && isSortable_4 ? c.InterestAmount.ToString() :
                                                           sortColumnIndex == 4 && isSortable_5 ? c.Post.ToString() :
                                                           sortColumnIndex == 5 && isSortable_6 ? c.Remarks :
                                                           "");
        ...
        public ActionResult Create()
        {
            ...
            vm.Operation = "add";
            return View("~/Areas/WPPF/Views/LoanMonthlyPayment/Create.cshtml", vm);

        }

        [HttpPost]
        public ActionResult CreateEdit(LoanMonthlyPaymentVM vm)
        {
        ...
        public ActionResult ReportView(int id)
        {
            try
            {
                ...
                dt.TableName = "dtLoan";
                rptLocation = AppDomain.CurrentDomain.BaseDirectory + @"Files\ReportFiles\WPPF\\rptLoanMonthlyPayment.rpt";
        ...
        public ActionResult reportVeiw(int id)
        {
            try
            {
                ...
                LoanMonthlyPaymentVM LoanMonthlyPaymentvm = new LoanMonthlyPaymentVM();
                PFReport report = new PFReport();
                LoanMonthlyPaymentvm = _repo.SelectAll(Convert.ToInt32(id)).FirstOrDefault();
                vm.Id = id;
                vm.Code = LoanMonthlyPaymentvm.Code;
                vm.TransType = AreaTypeWPPFVM.TransType;
                return PartialView("~/Areas/WPPF/Views/LoanMonthlyPayment/reportVeiw.cshtml", vm);
        ...
        [HttpPost]
        public ActionResult LoanMonthlyPaymentReport(PFReportVM vm)
        {
            try
            {
                string ReportHead = "";
                ...
                string[] cValues = { vm.Code, vm.Id.ToString() == "0" ? "" : vm.Id.ToString(), Ordinary.DateToString(vm.DateFrom), Ordinary.DateToString(vm.DateTo), AreaTypeWPPFVM.TransType };
