...
namespace SymServices.WPPF
{
    public class BankChargeDAL
    {
             ...
             , SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            ...
            string sqlText = "";
            List<BankChargeVM> VMs = new List<BankChargeVM>();
            BankChargeVM vm;
            #endregion
            try
            {
...
";
        ...
        //==================SelectAll=================
        /// <summary>
        /// Retrieves a list of bank Charge from the database, optionally filtered by ID or additional conditions.
        /// Supports external SQL connection and transaction handling for reuse within larger operations.
        /// </summary>
        /// <param name="Id">Optional ID to filter a specific bank Charge.</param>
...
";
        ...
        /// <summary>
        /// Inserts a new bank Charge record into the database with the provided details from the view model.
        /// Handles optional SQL connection and transaction management for use in broader transactional operations.
        /// </summary>
        /// <param name="vm">The <see cref="bankChargeVM"/> containing the bank Charge data to insert.</param>
        ...
        /// [5] = Method name ("InsertbankCharge").
        /// </returns>
        public string[] Insert(BankChargeVM vm, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            #region Initializ
            string sqlText = "";
            int Id = 0;
            ...
            retResults[5] = "InsertBankCharge"; //Method Name
            SqlConnection currConn = null;
            SqlTransaction transaction = null;
            int transResult = 0;
            #endregion
            ...
            try
            {
                ...
                vm.Id = _cDal.NextId("BankCharge", currConn, transaction);
                if (vm != null)
                {
                    string NewCode = new CommonDAL().CodeGenerationPF(vm.TransType, "BankCharge", vm.TransactionDate, currConn, transaction);

                    vm.Code = NewCode;

                    ////vm.Code = "BC-" + (vm.Id.ToString()).PadLeft(4, '0');

                    sqlText = "  ";
                    sqlText += @" 
INSERT INTO BankCharge(
Id
,Code
,BankBranchId
,TransactionDate
...
";
                    ...
                    if (transResult <= 0)
                    {
                        ...
                        throw new ArgumentNullException("Unexpected error to update BankCharge.", "");
                    }
                }
                else
                {
                    retResults[1] = "This BankCharge already used!";
                    throw new ArgumentNullException("Please Input BankCharge Value", "");
                }
                #endregion Save
                #region Commit
                if (Vtransaction == null)
        ...
        /// <summary>
        /// Updates an existing bank Charge record in the database with values from the provided view model.
        /// Handles optional SQL connection and transaction to allow for usage in broader transaction scopes.
        /// </summary>
        /// <param name="vm">The <see cref="BankChargeVM"/> containing the updated bank branch information.</param>
        ...
        /// [5] = Method name ("BankChargeUpdate").
        /// </returns>
        public string[] Update(BankChargeVM vm, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            #region Variables
            string[] retResults = new string[6];
            retResults[0] = "Fail";//Success or Fail
            ...
            retResults[3] = "sqlText"; //  SQL Query
            retResults[4] = "ex"; //catch ex
            retResults[5] = "Employee BankCharge Update"; //Method Name
            int transResult = 0;
            string sqlText = "";
            SqlConnection currConn = null;
            SqlTransaction transaction = null;
            ...
            try
            {
                ...
                if (transaction == null) { transaction = currConn.BeginTransaction("UpdateToBankCharge"); }
                #endregion open connection and transaction
                ...
                if (vm != null)
                {
                    ...
                    if (transResult <= 0)
                    {
                        // throw new ArgumentNullException("BankCharge Update", vm.BranchId + " could not updated.");
                    }
                    #endregion Commit
                ...
                {
                    throw new ArgumentNullException("BankCharge Update", "Could not found any item.");
                }
                if (Vtransaction == null && transaction != null)
                {
                    transaction.Commit();
        ...
        /// <summary>
        /// Performs a soft delete (archives) of one or more bank Charge records based on the provided IDs.
        ...
        /// </returns>
        public string[] Delete(BankChargeVM vm, string[] ids, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            #region Variables
            string[] retResults = new string[6];
            retResults[0] = "Fail";//Success or Fail
            ...
            retResults[3] = "sqlText"; //  SQL Query
            retResults[4] = "ex"; //catch ex
            retResults[5] = "DeleteBankCharge"; //Method Name
            int transResult = 0;
            string sqlText = "";
            SqlConnection currConn = null;
            SqlTransaction transaction = null;
            ...
            try
            {
                ...
                if (transaction == null) { transaction = currConn.BeginTransaction("DeleteToBankCharge"); }
                #endregion open connection and transaction
                if (ids.Length >= 1)
                {
                    ...
                    {
                        retVal = _cDal.SelectFieldValue("BankCharge", "Post", "Id", ids[i].ToString(), currConn, transaction);
                    ...
                    if (transResult <= 0)
                    {
                        throw new ArgumentNullException("BankCharge Delete", vm.Id + " could not Delete.");
                    }
                    #endregion Commit
                ...
                {
                    throw new ArgumentNullException("BankCharge Information Delete", "Could not found any item.");
                }
                if (Vtransaction == null && transaction != null)
                {
                    transaction.Commit();
        ...
        /// <summary>
        /// Posts one or more bank charge records by updating their status in the database.  
        ...
        public string[] Post(string[] ids, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            ...
            retResults[3] = "sqlText"; //  SQL Query
            retResults[4] = "ex"; //catch ex
            retResults[5] = "PostBankCharge"; //Method Name
            SqlConnection currConn = null;
            SqlTransaction transaction = null;
            #endregion
            try
            {
                ...
                if (ids.Length >= 1)
                {
                    ...
                    for (int i = 0; i < ids.Length - 1; i++)
                    {
                        retResults = _cDal.FieldPost("BankCharge", "Id", ids[i], currConn, transaction);
                        if (retResults[0].ToLower() == "fail")
                        {
                            throw new ArgumentNullException("BankCharge Post", ids[i] + " could not Post.");
                        }
                    }
                    #endregion Update Settings
                }
                ...
                {
                    throw new ArgumentNullException("BankCharge Information Post - Could not found any item.", "");
                }
                #region Commit
        ...
        /// <summary>
        /// Retrieves a filtered report of bank charge records from the database, optionally joining with bank branch data.
        /// Filters are applied dynamically based on provided condition fields and their corresponding values.
        /// </summary>
        /// <param name="vm">An instance of <see cref="BankChargeVM"/> used for additional context (currently unused in SQL query).</param>
        /// <param name="conditionFields">An array of field names to be used as filtering conditions in the SQL query.</param>
        /// <param name="conditionValues">An array of values corresponding to <paramref name="conditionFields"/> for filtering results.</param>
        /// <param name="VcurrConn">Optional external SQL connection. If not provided, a new connection is created.</param>
        /// <param name="Vtransaction">Optional external SQL transaction. If not provided, a local transaction scope is used (though not committed in this method).</param>
        /// <returns>
        /// A <see cref="DataTable"/> containing the filtered result set of bank charge records, including associated bank branch names.
        /// </returns>
        /// <exception cref="ArgumentNullException">Thrown if there is a SQL or general error, with details embedded in the exception message.</exception>

        public DataTable Report(BankChargeVM vm, string[] conditionFields = null, string[] conditionValues = null, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            #region Variables
            SqlConnection currConn = null;
            SqlTransaction transaction = null;
