...
{
    public class LoanMonthlyPaymentDAL
    {
        ...
        //==================SelectAll=================
        /// <summary>
        /// Retrieves a list of WPPF Loan Monthly Payment from the database, optionally filtered by ID or additional conditions.
        /// Supports external SQL connection and transaction handling for reuse within larger operations.
        /// </summary>
        /// <param name="Id">Optional ID to filter a specific WPPF Loan Monthly Payment.</param>
        ...
        /// <returns>A list of <see cref="BankBranchVM"/> representing the WPPF Loan Monthly Payment matching the criteria.</returns>
            ...
            , SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            ...
            string sqlText = "";
            List<LoanMonthlyPaymentVM> VMs = new List<LoanMonthlyPaymentVM>();
            LoanMonthlyPaymentVM vm;
            #endregion
            try
            {
...
";
        ...
        //==================Insert =================
        // <summary>
        /// Inserts a new WPPF Loan Monthly Paymen record into the database with the provided details from the view model.
        /// Handles optional SQL connection and transaction management for use in broader transactional operations.
        /// </summary>
        /// <param name="vm">The <see cref="BankBranchVM"/> containing the WPPF Loan Monthly Paymen data to insert.</param>
        ...
        /// [5] = Method name ("InsertPFLoanMonthlyPaymen").
        /// </returns>
        public string[] Insert(LoanMonthlyPaymentVM vm, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            #region Initializ
            string sqlText = "";
            int Id = 0;
            ...
            try
            {
                ...
                vm.Id = _cDal.NextId("LoanMonthlyPayment", currConn, transaction);
                if (vm != null)
                {
                    string NewCode = new CommonDAL().CodeGenerationPF(vm.TransType, "MonthlyLoanPayment", vm.TransactionDate, currConn, transaction);

                    vm.Code = NewCode;

                    ////vm.Code = "LoanMonthlyPayment-" + (vm.Id.ToString()).PadLeft(4, '0');

                    sqlText = "  ";
                    sqlText += @" 
INSERT INTO LoanMonthlyPayment(
Id
,Code
,Amount
,InterestAmount
...
";
                ...
                {
                    retResults[1] = "This LoanMonthlyPayment already used!";
                    throw new ArgumentNullException("Please Input LoanMonthlyPayment Value", "");
                }
                #endregion Save
                #region Commit
                if (Vtransaction == null)
        ...
        //==================Update =================
        /// <summary>
        /// Updates an existing WPPF Loan record Monthly Paymen in the database with values from the provided view model.
        /// Handles optional SQL connection and transaction to allow for usage in broader transaction scopes.
        /// </summary>
        /// <param name="vm">The <see cref="BankBranchVM"/> containing the updated WPPF Loan Monthly Paymen information.</param>
        ...
        /// [5] = Method name ("MonthlyPaymen").
        /// </returns>
        public string[] Update(LoanMonthlyPaymentVM vm, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            #region Variables
            string[] retResults = new string[6];
            retResults[0] = "Fail";//Success or Fail
            ...
            try
            {
                ...
                if (vm != null)
                {
                    ...
                    sqlText = "";
                    sqlText = "UPDATE LoanMonthlyPayment SET";
                    ...
                    if (transResult <= 0)
                    {
                        // throw new ArgumentNullException("LoanMonthlyPayment Update", vm.BranchId + " could not updated.");
                    }
                    #endregion Commit
                ...
                {
                    throw new ArgumentNullException("LoanMonthlyPayment Update", "Could not found any item.");
                }
                if (Vtransaction == null && transaction != null)
                {
                    transaction.Commit();
        ...
        public string[] Post(string[] ids, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            ...
            retResults[3] = "sqlText"; //  SQL Query
            retResults[4] = "ex"; //catch ex
            retResults[5] = "LoanMonthlyPayment"; //Method Name
            SqlConnection currConn = null;
            SqlTransaction transaction = null;
            #endregion
            try
            {
                ...
                if (ids.Length >= 1)
                {
                    ...
                    {
                        retResults = _cDal.FieldPost("LoanMonthlyPayment", "Id", ids[i], currConn, transaction);
                        if (retResults[0].ToLower() == "fail")
                        {
                            throw new ArgumentNullException("LoanMonthlyPayment Post", ids[i] + " could not Post.");
                        }
                    }
                    #endregion Update Settings
                }
                ...
                {
                    throw new ArgumentNullException("LoanMonthlyPayment Post - Could not found any item.", "");
                }
