...
namespace SymServices.PF
{
    public class LoanDAL
    {
            ...
            , SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            ...
            string sqlText = "";
            List<LoanVM> VMs = new List<LoanVM>();
            LoanVM vm;
            #endregion
            try
            {
...
";
        ...
        //==================Insert =================
        public string[] Insert(LoanVM vm, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            #region Initializ
            string sqlText = "";
            int Id = 0;
            ...
            try
            {
                ...
                vm.Id = _cDal.NextId("Loan", currConn, transaction);
                if (vm != null)
                {
                    string NewCode = new CommonDAL().CodeGenerationPF(vm.TransType, "Loan", vm.TransactionDate, currConn, transaction);

                    vm.Code = NewCode;

                    ////vm.Code = "LOAN-" + (vm.Id.ToString()).PadLeft(4, '0');
...
";
                ...
                {
                    retResults[1] = "This Loan already used!";
                    throw new ArgumentNullException("Please Input Loan Value", "");
                }
                #endregion Save
                #region Commit
                if (Vtransaction == null)
        ...
        //==================Update =================
        public string[] Update(LoanVM vm, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            #region Variables
            string[] retResults = new string[6];
            retResults[0] = "Fail";//Success or Fail
            ...
            try
            {
                ...
                if (vm != null)
                {
                    ...
                    if (transResult <= 0)
                    {
                        // throw new ArgumentNullException("Loan Update", vm.BranchId + " could not updated.");
                    }
                    #endregion Commit
                ...
                {
                    throw new ArgumentNullException("Loan Update", "Could not found any item.");
                }
                if (Vtransaction == null && transaction != null)
                {
                    transaction.Commit();
        ...
        public string[] Post(string[] ids, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            ...
            try
            {
                ...
                if (ids.Length >= 1)
                {
                    ...
                    {
                        retResults = _cDal.FieldPost("Loan", "Id", ids[i], currConn, transaction);
                        if (retResults[0].ToLower() == "fail")
                        {
                            throw new ArgumentNullException("Loan Post", ids[i] + " could not Post.");
                        }
                    }
                    #endregion Update Settings
                }
